...
from combobatch.vendor.shambhala.progress_display import ProgressEvent, _QueueLike
...
# VENDOR FIX (§6 bug 5): upstream selected the script with two independent `if`
# blocks, so `fixed_clusters` overwrote the `skip_qn` choice and the *approved*
# A+E speed-up combination silently quantile-normalized twice — once in Python,
# once in Octave. The four combinations are now a total function over one table,
# and the fourth script exists rather than falling back to a wrong one.
_PIPELINE_SCRIPTS: dict[tuple[bool, bool], str] = {
    # (skip_qn, fixed clusters supplied)
...
def _select_pipeline_script(skip_qn: bool, use_fixed_clusters: bool) -> str:
    """
    Choose the Octave driver script for one combination of speed-up options.

    Parameters
    ----------
    skip_qn
        True when quantile normalization was already applied Python-side.
    use_fixed_clusters
        True when precomputed gene clusters are injected as ``FIXED_CLUSTERS``.

    Returns
    -------
    The bare filename of the ``.m`` script to source.
    """
    return _PIPELINE_SCRIPTS[(bool(skip_qn), bool(use_fixed_clusters))]
...
def run_octave_normalize(
    ...
    script_name_sync = _select_pipeline_script(skip_qn, fixed_clusters is not None)
...
def precompute_clusters_from_p(
    ...
    labels_0indexed = km.fit_predict(
        p_log2
    )  # cluster genes (rows); features = P columns
...
def run_octave_normalize_streaming(
    ...
    clusters_preamble = ""
    script_name = _select_pipeline_script(skip_qn, fixed_clusters is not None)
    if fixed_clusters is not None:
    ...
    stdin_thread = threading.Thread(
        target=_write_stdin, args=(proc, tsv_string), daemon=True
    )
    stdin_thread.start()
    ...
    try:
        ...
        proc.kill()
        raise OctaveExecutionError(f"Octave subprocess timed out after {timeout_s}s.")
