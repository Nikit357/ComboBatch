...
from combobatch.vendor.shambhala.progress_display import (
    ProgressEvent,
    _ProgressRenderer,
    _QueueLike,
)
from combobatch.vendor.shambhala import q_rescale as q_rescale_module
...
def _run_python_cublock_batch(
    ...
    from combobatch.vendor.shambhala.cublock_python import cublock_python
    ...
    for col_idx in range(nh):
        ...
        cublock_result = cublock_python(
            qn_pool, n_reps=30, k=k, random_seed=random_seed
        )
        result_cols.append(cublock_result[:, 0])

        if progress_queue is not None:
            # VENDOR FIX (§6 bug 6): upstream passed done/total/speed_s_per_sample,
            # none of which are ProgressEvent fields, so every progress-enabled
            # python_cublock run died with a TypeError here.
            elapsed = time.time() - t_sample_start
            progress_queue.put(
                ProgressEvent(
                    worker_idx=worker_idx,
                    sample_done=col_idx + 1,
                    sample_total=nh,
                    elapsed_s=elapsed,
                    is_final=(col_idx + 1 == nh),
                )
            )

    result_np = (
        np.column_stack(result_cols)
        if len(result_cols) > 1
        else result_cols[0][:, np.newaxis]
    )
...
def harmonize_parallel(
    ...
    if octave_scripts_dir is None:
        # VENDOR FIX: upstream looked one level *above* the package, which only
        # resolved under `pip install -e .` because octave/ was never packaged.
        # Here octave/ lives inside the package and ships as package_data.
        octave_scripts_dir = str(pathlib.Path(__file__).resolve().parent / "octave")
