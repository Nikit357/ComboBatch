...
def _render_row(
    label: str, done: int, total: int, elapsed_s: float, finished: bool
) -> str:
...
class _ProgressRenderer(threading.Thread):
    ...
    def _emit_plain(self, event: ProgressEvent) -> None:
        if event.sample_done % self._log_every == 0 or event.is_final:
            ...
            pct = (
                (event.sample_done / event.sample_total * 100)
                if event.sample_total > 0
                else 0.0
            )
    ...
    def _render_ansi(self) -> None:
        ...
        eta_s = remaining / rate if rate > 0 else 0.0
        pct = (
            (total_done / self._total_samples * 100) if self._total_samples > 0 else 0.0
        )
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if (
                self._use_ansi
                and self._dirty
                and (now - last_render) >= _RENDER_INTERVAL
            ):
                self._render_ansi()
                last_render = now
            if self._stop_flag:
